...
export default function ZnoRowCreateNew ({
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    const [isDateOfBirth, setIsDateOfBirth] = useState<Date>()
          ...
          <Button
            ...
            className={cn(
              "justify-start text-left font-normal",
               !isDateOfBirth && "text-muted-foreground"
            )}
          >
            <CalendarIcon className="mr-2 h-4 w-4" />
            {isDateOfBirth ? format(isDateOfBirth, "PPP", {locale: ru}) : <span>Выберите время*</span>}
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-auto p-0" align="start">
          <Calendar
            mode="single"
            selected={isDateOfBirth}
            onSelect={setIsDateOfBirth}
            initialFocus
            locale={ru}
          />
          <div className="p-2 flex justify-center border-t">
            <DateTimePicker date={isDateOfBirth} setDate={setIsDateOfBirth}/>
          </div>
        </PopoverContent>
      </Popover>
    </TableCell>
